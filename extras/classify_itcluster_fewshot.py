...
def main():
	...
	train_batch = train_batch.groupby(lname, group_keys=False).apply(lambda x: x.sample(min(len(x), 30)))
	print(train_batch)
	...
	for run in range(args.runs):
		...
		res_path = f"{dirname}/ItClusterfiltercellsfshot_" + str(run)
	...
	res_path = f"{dirname}/ItClusterfiltercellsfshot_mean"
	os.makedirs(f"{res_path}", exist_ok=True)
