...
options(future.globals.maxSize= 16*891289600)

# use_virtualenv("~/mohit/torch-cpu", required = TRUE)
use_condaenv("jind", required = TRUE)
py_config()
...
anchors <- FindTransferAnchors(reference = reference, query = query, dims = 1:30, reference.reduction = "pca", nn.method = "rann", k.filter = NA)
...
pkl <- import("pickle")
path = sprintf("%s/seurat4", dirname(args$train_path))
